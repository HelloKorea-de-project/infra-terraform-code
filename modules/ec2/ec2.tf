    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          "arn:aws:s3:::${var.s3_bucket_name}/*",
          "arn:aws:s3:::${var.project_name}-test-zone",
          "arn:aws:s3:::${var.project_name}-test-zone/*"
        ]
      }
    ]
...
resource "aws_instance" "airflow" {
  ...
  tags = {
    Name = "${var.project_name}-Airflow-Server-Instance"
  }
  
  user_data = templatefile("${path.module}/tpl/airflow_user_data.tpl", {
    ssh_setting = templatefile("${path.module}/tpl/ssh_setting.tpl", {
...
resource "aws_instance" "main_server" {
  ...
  vpc_security_group_ids = [aws_security_group.private_instances.id, aws_security_group.django.id]
  key_name               = aws_key_pair.private_key.key_name
  iam_instance_profile = aws_iam_instance_profile.ec2_profile.name
  ...
  user_data = templatefile("${path.module}/tpl/django_user_data.tpl", {
    ssh_setting = templatefile("${path.module}/tpl/ssh_setting.tpl", {
      ssh_public_key = tls_private_key.private_ssh_key.public_key_openssh}),
    aws_access_key_id = var.aws_access_key_id,
    aws_secret_access_key = var.aws_secret_access_key,
...
resource "aws_instance" "grafana_server" {
  ...
  subnet_id              = var.private_subnet_ids[1]
  vpc_security_group_ids = [aws_security_group.private_instances.id, aws_security_group.grafana.id]
  key_name               = aws_key_pair.private_key.key_name
  ...
  user_data = templatefile("${path.module}/tpl/grafana_user_data.tpl", {
    ssh_setting = templatefile("${path.module}/tpl/ssh_setting.tpl", {
      ssh_public_key = tls_private_key.private_ssh_key.public_key_openssh})
  })
}
...
resource "aws_instance" "redis_server" {
  ...
  instance_type          = "t3.micro"
  subnet_id              = var.private_subnet_ids[1]
  vpc_security_group_ids = [aws_security_group.private_instances.id, aws_security_group.redis.id]
  key_name               = aws_key_pair.private_key.key_name


  tags = {
    Name = "${var.project_name}-message-broker-EC2"
  }

  user_data = templatefile("${path.module}/tpl/redis_user_data.tpl",{})
...
resource "aws_instance" "dbt_server" {
  ami                    = "${var.basic_ec2_ami}"
  instance_type          = "t3.medium"
  subnet_id              = var.private_subnet_ids[1]
  ...
  tags = {
    Name = "${var.project_name}-DBT-Server"
  }

  user_data = templatefile("${path.module}/tpl/dbt_user_data.tpl", {
    production_redshift_host = var.production_redshift_host,
    production_redshift_user = var.production_redshift_user,
    production_redshift_password = var.production_redshift_password,
    production_redshift_db = var.production_redshift_db,
  })
...
resource "aws_lb_listener" "grafana" {
  ...
  tags = {
    Name = "${var.project_name}-grafana-listener"
  }
}
...
resource "aws_lb" "django_server" {
  name               = "${var.project_name}-django-alb"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.alb.id]
  subnets            = var.public_subnet_ids
...
resource "aws_lb_target_group" "django_server" {
...
resource "aws_lb_listener" "django_server" {
  load_balancer_arn = aws_lb.django_server.arn
  port              = "80"  
  protocol          = "HTTP"

  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.django_server.arn
  }
  tags = {
    Name = "${var.project_name}-django-listener"
  }
  
}

resource "aws_lb_target_group_attachment" "django_server" {
  target_group_arn = aws_lb_target_group.django_server.arn
...
resource "aws_instance" "airflow_first_worker" {
  ami                    = "${var.airflow_ec2_ami}"
  instance_type          = "t3.medium"
  subnet_id              = var.private_subnet_ids[0]
  vpc_security_group_ids = [aws_security_group.private_instances.id, aws_security_group.airflow.id]
  ...
  tags = {
    Name = "${var.project_name}-Airflow-Worker-1"
  }
